...
c
      ...
      integer :: ndof, ndoferrors, ndofybar, ndofdwal
      integer :: itmp, itmp2
...
c
     ...
     &     ithree,'integer'//char(0),iotype)
        ...
        if(ndofdwal .ne. 0) then 
          ...
          dwal(:,:) = -9.87654321e32
          allocate( qread1(nshg2,ndofdwal) )
          iqsiz=nshg2*ndofdwal
          call readdatablock(descriptor,fname1//char(0),qread1,iqsiz,
     &                   'double'//char(0),iotype)
          dwal(:,1:ndofdwal)=qread1(:,1:ndofdwal)
          deallocate(qread1)
        else
          if(myrank==0) then
...
c
     ...
     &                                  ndofyphbar, 'out'//char(0))
         ...
         ! dwal
          if(idwal == 1) then
            call commuMax (dwal, point2ilwork, ndofdwal, 'in '//char(0))
            call commuMax (dwal, point2ilwork, ndofdwal, 'out'//char(0))
            call mpi_barrier(mpi_comm_world, ierr)  ! make sure everybody is done with ilwork
          endif
      endif
...
c
     ...
     &                      MPI_DOUBLE,MPI_MIN, mpi_comm_world, ierr ) 
      ...
      if(idwal == 1) then
        do idof=1,ndofdwal
          call mpi_allreduce(maxval(dwal(:,idof)), globmax, 1, 
     &                      MPI_DOUBLE, MPI_MAX, mpi_comm_world, ierr ) 
          call mpi_allreduce(minval(dwal(:,idof)), globmin, 1, 
     &                      MPI_DOUBLE, MPI_MIN, mpi_comm_world, ierr ) 
          if (myrank == 0) then
            write(*,925) 'max/min dwal(',idof,'):',globmax,globmin
            write(*,*) ''
          endif
        enddo
      endif
