...
c
     ...
     &             cp, k_T, elemb_local_size(lcblkb(1,iblk):lcblkb(1,iblk+1)-1),
     &             evl)
c
...
c        
     ...
     &                     + shape(iel,nodlcl) * BCB(iel,n,2)
              ...
              do n = 1, nshlb
                 nodlcl = lnode(n)
                 if (ibcb_conv_p_norm .eq. 1) then     ! Changed on 05/14/2009 following Farhad's suggestion
...
c
        if (iDNS.eq.0.and.itwmod.ne.-1) then  ! Avoid zeroing the wall stress for DNS calculations (slip velocity)
...
c.... get necessary fluid properties (including eddy viscosity)
c
        call getdiff(dwl, yl, shape,  xmudmi, xlb,   rmu, rho, cp, k_T,
     &            elemb_local_size(lcblkb(1,iblk):lcblkb(1,iblk+1)-1),
     &            evl)
c
c.... calculate the integraton variables
c
        call e3bvar (yl,              shape,
     &               shdrv,           xlb,
     &               lnode,           WdetJb,
     &               bnorm,           pres,            
     &               u1,              u2,              u3,
