        subroutine itrBC (y,ac, banma, iBC, BC, iper, ilwork)
c
c----------------------------------------------------------------------
c
...
c 
        if (numpe > 1) then
           call commu (banma, ilwork, 1, 'mrk')
           call commu (banma, ilwork, 1, 'out')
...
c
        where (btest(iBC,ib))
          y(:,id) =  BC(:,ibb)
        endwhere
c....------------Two phase heat conduction Temp BC----------- !6/8/15 Mengnan
       if(isclr.eq.0.and.iLSet.eq.2) then 
         if(bubgrow.eq.1.or.bubboil.eq.1)then
         do i=1, nshg
!           if(banma(i,1).eq.2.0d0)then
!!         if (y(i,6).GT.0.0) then
              if (y(i,6).LE.0.0) then
              y(i,id) = 370.15 !373.15 ! Kelvin
              endif
!           elseif(banma(i,1).eq.1.0d0)then
!              if (y(i,6).LE.0.0) then
!              y(i,id) = 307.15 ! Kelvin
!              endif
!           endif
         enddo
         endif
       endif
c....----------------Bubble growth Interior Temp BC--------------!6/8/15 Mengnan
