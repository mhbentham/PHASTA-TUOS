...
!       write(*,*) 'stsWriteFreq',stsWriteFreq
!       write(*,*) 'ntout',ntout
!       write(*,*) 'nstep(itseq)',nstep(itseq)
...
!!!!ORIGINAL VERSION ALISA TROFIMA
      use     stats
      include "common.h"

      integer      itmp2
      character*50 fname, fmt, fnamer
      character*5  cname
      character*10  cname2, cname2nd
      real*8       outvec(nshg,19)
      integer irstin, isize, nitems, nitems2
...
c.... open the output file
      ...
      itmp2  = 1 
      if (lstep .gt. 0) itmp2 = int(log10(float(lstep)))+1
c     
      write (fmt,"('(''restart.'',i',i1,',1x)')") itmp2
      write (fname,fmt) lstep
      fname = trim(fname) // cname2(myrank+1) 

      if (numpe.gt.idirtrigger) then
        if(iMeshingTool.eq.1) then
           fname = trim(cname2nd(int(myrank/idirstep)))
     1  // "/" // trim(fname)
        elseif(iMeshingTool.eq.0) then
           fname = trim(cname2nd(int(myrank/idirstep)*idirstep))
     1  // "-set/" // trim(fname)
        else
           if(myrank.eq.master) write(*,*) 'Check iMeshingTool'
        endif
...
c.... open input files
c 
      call openfile(  fname,  'append?', irstin ) 
      fnamer = 'statistics'  
      isize = nshg*19
      nitems = 5
c    
...
c     .... write the statistics
      ...
      call writedatablock(irstin, fnamer,outvec, nitems2, 
     &     'double', iotype)
    
      call closefile( irstin, "append" )    
      
      
c      WRITE(*,*)" Writing ASCII file "
c      WRITE(*,*)" "

c      OPEN(868,FILE='stats.out-ascii-phasta-1.9.5_stats')
c      WRITE(868,*)" nshg  lstep  "
c      WRITE(868,*) nshg, lstep
c      DO I=1, nshg
c         WRITE(868,"(5(F24.8))")(outvec(I,J),J=1,19)
c      ENDDO
c      
c      CLOSE(868)
