      subroutine getDiff( dwl,yl, shape, xmudmi, xl, rmu,  rho, cp, k_T,
     &                    elem_size, evl)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &        shape(npro,nshl),   rho(npro), cp(npro), k_T(npro),
     &        dwl(npro,nshl),     sclr(npro), 
     &        xl(npro,nenl,nsd),  evl(npro,nshl)
      real*8 elem_size(npro)
      integer n, e

      real*8  epsilon_ls, kay, epsilon, epsilon_lst,
     &        h_param, prop_blend(npro),test_it(npro)
      real*8  epsilon_ls_tmp,epsilon_lsd_tmp, epsilon_lst_tmp
c
c    
...
c	if (myrank.eq.master) write(*,*) 'iLSet = ', iLSet
...
!-----------------------Thermal properties mixing---------------------
         if (solheat.eq.1)then
           do i = 1, npro
           epsilon_lst_tmp = epsilon_lst*elem_size(i)
!           epsilon_lst_tmp = epsilon_lst *
!     &           elem_local_size(lcblk(1,iblk)+i-1)
              if (sclr(i) .lt. - epsilon_lst_tmp) then
                 prop_blend(i) = zero
              elseif  (abs(sclr(i)) .le. epsilon_lst_tmp) then
                 prop_blend(i) = 0.5*(one + sclr(i)/epsilon_lst_tmp +
     &                           (sin(pi*sclr(i)/epsilon_lst_tmp))/pi)
              elseif (sclr(i) .gt. epsilon_lst_tmp) then
                 prop_blend(i) = one
              endif
           enddo
          cp = datmat(1,3,2)+(datmat(1,3,1)-datmat(1,3,2))*prop_blend
     &
          k_T = datmat(1,4,2)+(datmat(1,4,1)-datmat(1,4,2))*prop_blend
!        write(*,*)'rho,cp,k_T',rho,cp,k_T
         endif
...
c      
      if (iLES .gt. 0 .and. iRANS.eq.0) then   ! simple LES
        ...
        call AddEddyViscDNS(yl, shape, rmu, evl)
...
!this was made messy by the addEddyVisc routines  Must clean up later.
      ...
      subroutine getdiffsclr(shape, dwl, yl, diffus,evl)

      use turbSA
      use turbKE ! access to KE model constants
      include "common.h"
      
      real*8   diffus(npro), rho(npro), cp(npro), k_T(npro),
     &         elem_size(npro), rmu(npro),  xmudmi(npro,ngauss),
     &         xl(npro,nenl,nsd), evl(npro,nshl)
     ...
     &                    elem_size, evl) ! Mengnan 2/23/2016

      if(isclr.eq.0) then  ! solving the temperature equation
         diffus(:) = datmat(1,4,1)/(datmat(1,1,1)*datmat(1,3,1))
...
c in all of these cases, eddy viscosity is calculated normally
      ...
      subroutine AddEddyViscDNS(yl,shape,rmu,evl)
      use turbSA
      include "common.h"
c     INPUTS
      double precision, intent(in), dimension(npro,nshl,ndof) ::
