...
c used locally
     ...
     &        sclr_ls(npro),
     &        u1_tmp(npro),   u2_tmp(npro),  u3_tmp(npro)
c    Kay-Epsilon
...
c... Put a source in the interface to mitigate the effect of mass loss
c... associated with level set method
            do i = 1,npro

                  u1_tmp(i) = u1(i) + C_int_adjust * gradS(i,1) ! KEEP
                  u2_tmp(i) = u2(i) + C_int_adjust * gradS(i,2)
                  u3_tmp(i) = u3(i) + C_int_adjust * gradS(i,3)
            enddo
         ...
         elseif (isclr.eq.2) then !we are redistancing level-sets
            ...
            srcR = sign_levelset
            srcL =  zero   
        elseif (isclr.eq.0) then   ! Energy Equation when Level Set is active
         ...
         umod(:,1) = u1      ! These are for the LHS
         umod(:,2) = u2
         umod(:,3) = u3 
...
c For Level Set Scalar 1
      if (isclr.eq.1)  then
        do n = 1, nshl
           u1   = u1   + shape_funct(:,n) * yl(:,n,2)
           u2   = u2   + shape_funct(:,n) * yl(:,n,3)
           u3   = u3   + shape_funct(:,n) * yl(:,n,4)
        enddo
      elseif (isclr.eq.2) then !we are redistancing level-sets
...
c                  
     ...
     &             elem_local_size(lcblk(1,iblk)-1+ii)
