...
!      call drvLesPrepDiag(diag,ilwork,iBC,BC,iper,rowp,colm,lhsK,lhsP)
     ...
     &                   MPI_COMM_WORLD,ierr)
      ...
      step = 1
      write(*,*)myrank,'start to calc eigen at level:',level
      tramg_ev = 0
...
!     &                     amg_nnz(level),twork2,twork1,1)
     ...
     &         lworkl,ierr) 
          write(*,*)'mcheck: mls:',ierr,'iconv:',iparam(5)
      end if
      evmax = dr(1)
      if (myrank.eq.0) then
         write(*,*)'largest eigenvalue of PPE: ',evmax
      endif
     ...
     &                          ilwork,BC,iBC,iper)
      ...
      write(*,*)'mcheck:levelx',ramg_levelx
      do lvl=1,ramg_levelx-1
...
!***********************************************************
!      ramg_mls_apply: u = MLS(v,(lhs,b))
!      v : error vector, Ax-b
!***********************************************************

      subroutine ramg_mls_apply(u,v,b,level,colm,rowp,lhsK,lhsP,
     &                          ilwork,BC,iBC,iper,
     &                          resin,resout,fwdbck)
      use ramg_data
      include "common.h"
      ...
      real(kind=8),intent(in),dimension(amg_nshg(level)) :: v,b
      real(kind=8),intent(inout),dimension(amg_nshg(level)) :: u
...
!      if (.false.) then ! always post smoother
      if (.true.) then ! always pre smoother
          call ramg_mls_apply_fwd(u,v,b,level,colm,rowp,lhsK,lhsP,
     &                  ilwork,BC,iBC,iper)
      else
          call ramg_mls_apply_post(u,v,b,level,colm,rowp,lhsK,lhsP,
     &                  ilwork,BC,iBC,iper)
      endif
          u = 1.1*u
      ...
      subroutine ramg_mls_apply_fwd(u,v,b,level,colm,rowp,lhsK,lhsP,
     &                          ilwork,BC,iBC,iper)
      use ramg_data
      include "common.h"
      include "mpif.h"
      ...
      real(kind=8),intent(inout),dimension(amg_nshg(level)) :: v,b
      real(kind=8),intent(inout),dimension(amg_nshg(level)) :: u
      ...
      do i=2,mlsDeg
         ! c_i*A*(A*...*r)
!        call ramg_calcAv(amg_A_colm(level)%p,amg_A_rowp(level)%p,
!     &          amg_A_lhs(level)%p,amg_nshg(level),amg_nnz(level),
!     &          res,pAux,0)
     ...
     &        ilwork,BC,iBC,iper,1)
      ...
      subroutine ramg_mls_apply_post(u,v,b,level,colm,rowp,lhsK,lhsP,
     &                          ilwork,BC,iBC,iper,
     &                          resin,resout)
      use ramg_data
      include "common.h"
      ...
      call ramg_mls_apply_fwd(y1,v,b,level,colm,rowp,lhsK,lhsP,
     &     ilwork,BC,iBC,iper,
     &     resin,resout)
!      y1=1.1*y1
