...
!     &              amg_A_lhs(level1)%p,
     ...
     &              amg_nshg(level2))
      ...
      if (ramg_verb.gt.2) then
      write(*,*)'level:',level2,' nshg:',amg_nshg(level2),
     &          ' nnz:',amg_nnz(level2)
      endif
     ...
     &               amg_A_rhs(level2)%p)
      ...
      write(*,7101)'IT*A time: ',cpusec(2)-cpusec(1)
      write(*,7101)'IT*A * I colm: ',cpusec(3)-cpusec(2)
      write(*,7101)'IT*A * I rowp: ',cpusec(4)-cpusec(3)
      write(*,7101)'IT*A * I value: ',cpusec(5)-cpusec(4)
      write(*,7102)cpusec(10)-cpusec(1),level2
7101  format(A,f9.2,' sec')
7102  format(/' *** total I^T A I time:',f9.2,
     & ' sec at level ',i5)
      endif

      IF (.false.) THEN
          do i=1,amg_nshg(level2)
             p = amg_A_colm(level2)%p(i)
             q = amg_A_rowp(level2)%p(p)
             if (q.ne.i) then
                 write(*,*) 'not right 1'
             endif
             if (amg_A_lhs(level2)%p(p,1).ne.1) then
                 write(*,*) 'not right 2',amg_A_lhs(level2)%p(p,1)
             endif
          enddo
      END IF
