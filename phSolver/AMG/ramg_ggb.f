     ...
     &             i,j,ishifts,maxitr,model,nconv
      ...
      if (ramg_ggb_flag.ne.0) return

!      if (ramg_flag.ne.0) return
     ...
     &                   MPI_COMM_WORLD,ierr)
      ...
      step = 1
      write(*,*)myrank,'AMG GGB: calculating eigenvalue/vectors'
      tramg_ev = 0
...
!     &                     amg_A_lhs(1)%p,amg_nshg(1),
!     &                     amg_nnz(1),teA(:,i),ramg_ggb_ev(:,i),0)
...
!             enddo
      ...
      subroutine ramg_ggb_test_av(nx,v,w,mtx,mtxmaxn)
      integer :: nx,mtxmaxn
      real(kind=8) :: mtx(mtxmaxn,mtxmaxn)
      real(kind=8),dimension(nx) :: v,w
      ...
      end subroutine !ramg_ggb_test_av
...
!     &               ramg_sol,ramg_rhs,
     ...
     &                   ilwork,BC,iBC,iper,1)
      ...
      call ramg_V_cycle(ramg_sol,vF,ramg_r_i,ramg_r_o,10,1,
     &                  colm,rowp,lhsK,lhsP,ilwork,BC,iBC,iper)
     ...
     &           )
      ...
      !arguments
      integer, intent(in)         :: ramg_mode,clevel
!      real(kind=8) :: ramg_sol(nshg),ramg_rhs(nshg)
     ...
     &               :: ramg_sol,ramg_rhs
      ...
      real(kind=8),dimension(amg_nshg(clevel)) :: ggberr,ggberr1
      real(kind=8),dimension(maxnev,maxnev) :: mtxggbA
      integer,dimension(maxnev) :: indx
      real(kind=8) :: ramg_r_i,d
      integer :: i,j,k,p,q,m,n,ii,jj,kk
...
!          do i=1,maxnev
!             call MPI_Allreduce(vR(i),rtemp,1,
!    &            MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
!          enddo
      vR = rtemp
      end if
...
!      call ramg_dump(rtemp,maxnev,'vR2para   ')
      !stop
      mtxggbA = ramg_ggb_eA
!      call ramg_dump_rn(mtxggbA,maxnev,maxnev,'matrixAggb')
      call ramg_nr_ludcmp(mtxggbA,maxnev,maxnev,indx,d)
      call ramg_nr_lubksb(mtxggbA,maxnev,maxnev,indx,vR)
      ...
      end subroutine !ramg_ggb_V_cycle
