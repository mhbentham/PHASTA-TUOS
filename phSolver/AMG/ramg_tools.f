      ...
      do i=1,amg_nshg(level)
         ...
         do k=amg_A_colm(level)%p(i),amg_A_colm(level)%p(i+1)-1
            ...
            if (.true.) then
                ...
                if ( (myrank.ne.1).or.(mi.ne.1) ) then
                !write(*,*)'mcheck smoother,',myrank,mi,mj
                endif
            endif
            endif
         enddo
...
!***********************************************
      ...
      subroutine ramg_dot_p(v,u,redun,norm)
      include "common.h"
      include "mpif.h"
      include "auxmpi.h"
      
      integer :: redun
      real(kind=8),intent(in),dimension(nshg,redun) :: v,u
...
!************************************************************      
      ...
      subroutine ramg_L2_norm_p(v,vflow,norm)
      include "common.h"
      include "mpif.h"
      include "auxmpi.h"
      
      integer,intent(in) :: vflow
      real(kind=8),intent(in),dimension(nshg,vflow) :: v
      real(kind=8),intent(inout)    :: norm
      integer                       :: i,k
      real(kind=8) :: normt
      normt = 0
...
7002  format(/' **** ramg: Deallocation error at level',i5)
      ...
      call mgetpid(mpid)
      write(mformat,*)'ps -p ',mpid,' -o rss= -o vsz= >> ',mfname
      !write(*,*)mformat
      call msystem(mformat)
      end subroutine ! ramg_check_mem

      subroutine mgetpid(mpid)
      use ifport
      integer mpid
      mpid = getpid()
      end subroutine

      subroutine msystem(mcomm)
      use ifport
      character*80 mcomm
      integer e
      e = system(mcomm)
