...
!**********************************************************
!      ramg_lhs_scale *** GLOBAL SCALE ***
!      scale lhsK,lhsP , make diagonal entries all 1
!**********************************************************
      subroutine ramg_lhs_scale(colm,rowp,lhsK,lhsP,
     &                    nshg,nnz_tot,nflow,res,flowDiag,
     &                    ilwork,nlwork,ndofBC,BC,iBC,iper)
      implicit none
      !***********parameters*************
      ...
      !***************WANNA MODIFY THESE*********
      real(kind=8),intent(inout),dimension(9,nnz_tot)  :: lhsK
      real(kind=8),intent(inout),dimension(4,nnz_tot)  :: lhsP
      real(kind=8),intent(inout),dimension(nshg,nflow) :: res
      !** boundary info **
      ...
      real(kind=8),intent(inout),dimension(nshg,4)     :: flowDiag
      ...
      do i = 1,nshg
         do j = colm(i),colm(i+1)-1
            ...
            do m = 1,3
               do n = 1,3
                  lhsK((m-1)*3+n,j) = lhsK((m-1)*3+n,j) *
     &             flowDiag(i,m)*flowDiag(k,n)
               end do
            end do
            ! scale C
            lhsP(4,j) = lhsP(4,j)*flowDiag(i,4)*flowDiag(k,4)
            ! scale G
            do m = 1,3
               lhsP(m,j) = lhsP(m,j)*flowDiag(k,m)*flowDiag(i,4)
            end do
         end do
         ! scale res
         do m = 1,4
            res(i,m) = res(i,m) * flowDiag(i,m)
         end do
      end do

      end subroutine! end ramg_lhs_scale
...
!      write(*,*)'mcheck incomplete: ',j
      ...
      do i = 1,nshg
         ...
         do m = lhsGPcolm%p(i),lhsGPcolm%p(i+1)-1
            j = lhsGProwp%p(m)
            rtemp = rtemp + lhsGP%p(1,m)*mrhs(j,1)*mflowdiag(j,1)**2
     &        + lhsGP%p(2,m)*mrhs(j,2)*mflowdiag(j,2)**2
     &        + lhsGP%p(3,m)*mrhs(j,3)*mflowdiag(j,3)**2
         enddo
         amg_A_rhs(1)%p(i) = mrhs(i,4) - rtemp
      enddo
...
!     &             'A0        ')
     ...
     &         i,'  ',rtp,'  ',rtn,'  ',rtemp
      ...
      if (iamg_scale.eq.0) then ! original ( noscale )
         amg_ppeDiag(1)%p(:) = 1
      else if (iamg_scale.eq.1) then ! scaled by diag(C)...
         amg_ppeDiag(1)%p(:) = mflowDiag(:,4)
      else if (iamg_scale.eq.2) then ! scaled by diag(PPE) 1...1
       do i=1,nshg
         amg_ppeDiag(1)%p(i) = 
