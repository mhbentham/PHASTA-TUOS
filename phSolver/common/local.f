...
c
        ...
        subroutine localBC (globaliBC, globalBC, rlocaliBC, rlocalBC,
     &                      ientmp,n,code)
c
c----------------------------------------------------------------------
c
...
c
c input:
c  globaliBC (nshg,n)             : global iBCredist array
c  globalBC (nshg,n)              : global BCredist array
c  rlocaliBC (npro,nshl,n)        : local iBCredist array
c  rlocalBC (npro,nshl,n)         : local BCredist array
c  ien    (npro,nshl)             : nodal connectivity
c  n                              : number of d.o.f.'s to be copied
c  code                           : the transfer code
c                                  .eq. 'gather  ', from global to local
...
c
        ...
        real*8    globalBC(nshg,n),         rlocalBC(npro,nshl,n)  
        integer   globaliBC(nshg,n),        rlocaliBC(npro,nshl,n),
     &            ien(npro,nshl),           ientmp(npro,nshl)
c
        character*8 code
...
c
          ...
          do j = 1, n
            do i = 1, nshl
              rlocaliBC(:,i,j) = globaliBC(ien(:,i),j)
              rlocalBC(:,i,j) = globalBC(ien(:,i),j)
            enddo
          enddo
...
c            do i = 1, nshl
c              do nel = 1,npro
cc first check that another local node hasn't already set the BC value
cc if so, then take the smallest of the two
c                if (rlocaliBC(nel,i,j) .gt. 0) then
c                  if (globaliBC(ien(nel,i),j) .gt. 0) then
c                    if (abs(rlocalBC(nel,i,j)) .lt. 
c     &                  abs(globalBC(ien(nel,i),j))) then
c                      globalBC(ien(nel,i),j) =  rlocalBC(nel,i,j)
c                    endif
c                  else
c                     globalBC(ien(nel,i),j) =  rlocalBC(nel,i,j)
c                  endif
c                  globaliBC(ien(nel,i),j) = rlocaliBC(nel,i,j)
c                else
c                  globalBC(ien(nel,i),j) =  rlocalBC(nel,i,j)
c                  globaliBC(ien(nel,i),j) = rlocaliBC(nel,i,j)
c                endif
...
c
          do j = 1, n
            do i = 1, nshl
              do nel = 1,npro
c First check if global node has been previously set and
c then check if new BC value is smaller (i.e. closer to interface)
c than current value
                if (globaliBC(ien(nel,i),j) .gt. 0) then   
c if iBC local > iBC global then the local trumps the global
                  if (rlocaliBC(nel,i,j).gt.globaliBC(ien(nel,i),j))
     &            then
                    globalBC(ien(nel,i),j) =  rlocalBC(nel,i,j)
                    globaliBC(ien(nel,i),j) = rlocaliBC(nel,i,j)
c if iBC local = iBC global then must compare BC values and take the smallest
                  elseif 
     &             ((rlocaliBC(nel,i,j).eq.globaliBC(ien(nel,i),j))
     &             .and.
     &             (abs(rlocalBC(nel,i,j)) .lt.
     &              abs(globalBC(ien(nel,i),j))) ) then
                     globalBC(ien(nel,i),j) =  rlocalBC(nel,i,j)
                     globaliBC(ien(nel,i),j) = rlocaliBC(nel,i,j)
                  endif
                else
...
c
        ...
        subroutine locali (iglobal, ilocal, ientmp, n, code)
c
c----------------------------------------------------------------------
c
c This subroutine performs a vector gather/scatter operation.
c
c input:
c  iglobal (nshg,n)             : global integer array
c  ilocal (npro,nshl,n)         : local integer array
c  ien    (npro,nshl)      : nodal connectivity
c  n                            : number of d.o.f.'s to be copied
c  code                         : the transfer code
c                                  .eq. 'gather  ', from global to local
...
c
        ...
        dimension iglobal(nshg,n),          ilocal(npro,nshl,n),
     &            ien(npro,nshl),           ientmp(npro,nshl)
c
        character*8 code
...
c
          do j = 1, n
            do i = 1, nshl
              do nel = 1,npro
                iglobal(ien(nel,i),j) = iglobal(ien(nel,i),j)
     &                                + ilocal(nel,i,j)
              enddo
            enddo
          enddo
...
c
          do j = 1, n
            do i = 1, nshl
              do nel = 1,npro
                iglobal(ien(nel,i),j) = ilocal(nel,i,j)
              enddo
            enddo
          enddo
c
