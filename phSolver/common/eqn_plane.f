...
c
 	...
 	write(*,*) '***** Check if the mesh has been tetrahedralized !'      
c
...
c
	...
	enddo
	npin = j
	 write(*,*) 'inlet plane has ', npin, ' nodes'
...
c	aR = xnrml*xA(1)+ynrml*xA(2)+znrml*(xA(3)+rcydist) 
	aR = aI + plandist
c	write(*,*) 'eqn plane: aI, aR, plandist = ', aI, aR, plandist
...
c
	...
	allocate (ien2D(itmp,4))
     ...
     &		- ynrml*x(ien(i,k),2) - znrml*x(ien(i,k),3) 
	    ...
	    do j=1,nshl    ! if the two nodes of the element are on different sides of the plane > this element
c   				is cut by the plane  !!
	      do l=j,nshl
	        if (erreur(j)*erreur(l).le. 0.0e0) then
 	          nelint=nelint+1
		  ien2D(nelint,:) = ien(i,:)
		  goto 95
	        endif
