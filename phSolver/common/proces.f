...
c        write(*,*) 'First line in process, l. 46'
...
c
c         write(*,*) 'myrank, master: ', myrank, master
c        if (myrank.eq.master) write(*,*) 'gendat is to start'
        call gendat (y,              ac,             banma,
     &               point2x,        iBC,            BC,
     &               point2iper,     point2ilwork,   shp,
     &               shgl,           shpb,           shglb,
     &               point2ifath,    velbar,         point2nsons )
c	if (myrank.eq.master) write(*,*) 'gendat is done' 
        call setper(nshg)
...
c
	if (myrank.eq.master)
     &	write(*,*) 'iDNS, itwmod, iLES = ', iDNS, itwmod, iLES
...
c
	if (myrank.eq.master) write(*,*) 'calling itrdrv! '
