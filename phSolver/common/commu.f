...
c                      = 'mrk' for communicating the marker
c                        array (banma), Created by Jun, Oct,2013
c
c---------------------------------------------------------------------
c
...
c
      ...
      integer imrk      !Just a temporary integer used in marker
                        !communication
     ...
     &          ilwork(nlwork)
      ...
      if (code .ne. 'in ' .and. code .ne. 'out' .and. code .ne. 'mrk')
     &  call error ('commu   ','code    ',0)
      ...
      elseif (n .eq. nflow*nsd ) then   !surface tension + qres
         kdof = 15 
      elseif (n .eq. (nflow-1)*nsd + 10 ) then   ! qres + diss tensor (timeseries output)
         kdof = 16
      elseif (n .eq. nflow*nsd + 10 ) then   ! qres + surf tension + diss tensor (timeseries output)
         kdof = 17
      else
...
c
        if (iacc .EQ. 0) then  
c
c.... residual communication and marker communication
c
          if (code .eq. 'in ' .or. code .eq. 'mrk') then
...
c    Note that this involves splitting up the chunk of received data
...
c... The following algorithm is used to compare the marker values of
c    common points between different processors and then use the larger
c    markers for the specific common points.

      if(code .eq. 'mrk') then
        itkbeg=1
        jdl=0
        do j=1,numtask         ! time to do all the segments that needed
                               ! to be assembled into the global vector
          iacc   = ilwork (itkbeg + 2)
          numseg = ilwork (itkbeg + 4)
          isgbeg = ilwork (itkbeg + 5)
          if(iacc.eq.1) then
            jdl=jdl+1        ! this time we are dealing with markers

c... The comparing process works here
            itemp = 1
            do idof = 1,n
              do is = 1,numseg
               isgbeg = ilwork (itkbeg + 3 + 2*is)
