...
c using a turbulence wall model
      ...
      effvisc = zero
c	if (myrank.eq.master) write(*,*) 'itwmod genbc = ', itwmod
c
c
c.... ----------------------> Wall Normals  <--------------------------
...
c
      ...
      call genwnmp (iBC,  BCtmp,   x,   ilwork, iper, wnrmp)
c
c  determine the first point off the wall for each wall node
c
      call genotwn (x,BCtmp, iBC, nsurf, wnrmp)
c
c.... ------------------------>  Conversion  <-------------------------
c
...
c            write(*,*) 'Number of Surface_ID = ', nsidg
!	    write(*,*) 'nelblb, npro = ', nelblb, npro
	end if
...
c We want to print out SurfID coordinates even if they are not on a modeled surface:
c  (btest condition has been moved downstream by Igor)
...
c Igor, 12/28/2009. Print out the coordinates of SurfID = 10 boundary nodes:
c		write(*,*) 'myrank, sid = ', myrank, sid
		if (sid.eq.10) then
		 if (inew.eq.1) then
           open(unit=1101, file='../bctinflow.dat', status='old')
		   inew = 0
		 end if
		  do j12 = 1, nenbl  ! Number of nodes on the boundary face 
		    write(1101, '(I6, 10E15.7)') myrank, x(ienb(j12), :) 
!                    write(1101, '10E15.9)') 0.0, x(ienb(j12), 2:3) 
		  end do
		end if
...
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 266'
	if (itvn.gt.0) close(1101)
...
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 288'

            if (numpe.gt.1) then
c.... add areas and norms contributed from other processors
c         if (myrank.eq.master) write(*,*) 'genbc.dat l. 292'
!          if (myrank.eq.master) write(*, '(40F6.2)') wnrm
!         if (myrank.eq.master) write(*,*) '************************************'
!          if (myrank.eq.master) write(*, '(40F6.2)') ilwork(1:20)
!         if (myrank.eq.master) write(*,*) '************************************'
              call commu (wnrm, ilwork, 3, 'in ')
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 294'
            endif
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 296'
...
c.... local periodic (and axisymmetric) boundary conditions (no communications)
            wnmdb=wnrm
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 302'
...
c        if (myrank.eq.master) write(*,*) 'genbc.dat l. 316'
            do nn = 1, nshg
               if(nsurf(nn).eq.1) 
...
cc        if (myrank.eq.master) write(*,*) 'genbc.dat l. 327'
c     
...
c$$$           endif
c$$$        enddo
       if (myrank.eq.master) write(*,*) 'genbc is done.'
c
c.... return
c
      return
...
c
            do iblk=1, nelblb   ! loop over boundary element blocks
               ...
               do i = 1, npro   ! loop over boundary elements
                  ...
                  iBCB2=miBCB(iblk)%p(i,2)
                  ienb(1:nshl)=mienb(iblk)%p(i,1:nshl)
c Only consider elements with natural pressure boundaries
                  if (.not.(btest(iBCB1,1)))  cycle
c     
...
c
            do nn = 1, numnp       ! loop over all nodes
c leave nodes without natural pressure bc
              if(npbc(nn) .eq. 0) cycle
              e1 = wnrmp(nn,:)
              wmag=e1(1)*e1(1)+e1(2)*e1(2)+e1(3)*e1(3)
              wmag=1./sqrt(wmag)
              e1=e1*wmag   
...
c
      ...
      subroutine genotwn(x, BCtmp, iBC, nsurf, wnrmp)
c
c----------------------------------------------------------------------
...
c
      do iblk=1, nelblb         ! loop over boundary element blocks
         ...
         do i = 1, npro         ! loop over boundary elements
            ...
            iBCB2=miBCB(iblk)%p(i,2)
            ienb(1:nshl)=mienb(iblk)%p(i,1:nshl)
            if (btest(iBCB1,4) .or. btest(iBCB1,1)) then ! (on the wall or pressure bc)
...
c                     if(nsurf(nn).ne.0) cycle
c                               !       don't bother with no-slip nodes
...
c                              !       calculate dot product with wnrm
                     if (btest(iBCB1,1)) then
                       vec(:)=vec(:)*wnrmp(ienb(nod),:)
                     else
