...
c  The maximum number of bubbles for mpid is specified here:
        parameter     ( maxbubs = 5 )
...
c from different modules
	...
	common /amgvari/irun_amg,irun_amg_sa,irun_amg_prec,
     ...
     &  tvbcswitch,ibcb_conv_p, ibcb_conv_p_norm, itvbc
	common /pcboiling/ bubboil,solheat, bubgrow, h_fg, T_sat, epsilon_lst,
     &                     delt_T(4), numshell(4,2), bubble_vol(4), 
     &                     bubble_tempG(4), bubbleID(4)
        common /contactangle/ CA_flag, theta_adv, theta_rec, Forcecont, 
     &                        stretch, Fapp_thick, Fapp_heigh
        common /sequence/ seqsize, stepseq(100)
	common /workfc/ master, numpe, myrank
	common /fronts/ maxfront, nlwork, idirstep, idirtrigger
        integer*8 numper, nshgt, nshg0
	...
	common /timer4/ birth, death, comtim
     ...
     &                   modlstats, idis, nohomog, ierrsmooth
        common /mpistats/iISend, iIRecv, iWaitAll, iAllR,
     &                   impistat, impistat2, rmpitmr,
     &                   rISend, rIRecv, rWaitAll, rAllR, rCommu
...
c
     ...
     &          coalcon_rem, icoalCtrl, icoalcon_verbo
     ...
     &                   C_int_cap, epsilonBT, 
     &                   xdistancesum(maxbubs), ydistancesum(maxbubs), zdistancesum(maxbubs),
     &                   totalxdist(maxbubs), totalydist(maxbubs), totalzdist(maxbubs),
     &                   avgxdistance(maxbubs), avgydistance(maxbubs), avgzdistance(maxbubs),
     &                   avgxdistold(maxbubs), avgydistold(maxbubs), avgzdistold(maxbubs),
     &                   xdistideal(maxbubs), ydistideal(maxbubs), zdistideal(maxbubs),
     &                   dx_new(maxbubs), dy_new(maxbubs), dz_new(maxbubs), ddxvel(maxbubs), 
     &                   ddyvel(maxbubs), ddzvel(maxbubs),
     &                   xvelsum(maxbubs), yvelsum(maxbubs), zvelsum(maxbubs),
     &                   totalxvel(maxbubs), totalyvel(maxbubs), totalzvel(maxbubs),
     &                   avgxvel(maxbubs), avgyvel(maxbubs), avgzvel(maxbubs),
     &                   avgxvelold(maxbubs), avgyvelold(maxbubs), avgzvelold(maxbubs),
     &                   velwghtsum(maxbubs), totalvelwght(maxbubs), bubvolsum(maxbubs),
     &                   totbubvol(maxbubs), denssum(maxbubs), totbubdens(maxbubs),
     &                   xcforcesum(maxbubs), ycforcesum(maxbubs),
     &                   zcforcesum(maxbubs), totalxcforce(maxbubs),
     &                   totalycforce(maxbubs), totalzcforce(maxbubs),
     &                   totalxcforceold(maxbubs), totalycforceold(maxbubs), totalzcforceold(maxbubs),
     &                   avgxcforce(maxbubs), avgycforce(maxbubs), avgzcforce(maxbubs), avgxcforceold(maxbubs),
     &                   avgycforceold(maxbubs), avgzcforceold(maxbubs), avgxcf(maxbubs),
     &                   avgycf(maxbubs), avgzcf(maxbubs),
     &                   x_c_f(maxbubs), y_c_f(maxbubs), z_c_f(maxbubs),
     &                   xforcenewtsum(maxbubs), yforcenewtsum(maxbubs),
     &                   zforcenewtsum(maxbubs), totxfnewtsum(maxbubs),
     &                   totyfnewtsum(maxbubs), totzfnewtsum(maxbubs),
     &                   xcfnewtons(maxbubs), ycfnewtons(maxbubs), zcfnewtons(maxbubs), rholiq, rhogas,     
     &                   coalbubrad, coalcon_dist,
     &                   avgxcoordold(100), avgycoordold(100),
     &                   avgzcoordold(100),
     &                   i_res_cf, nzinBsum(maxbubs), ntotnzinB(maxbubs),
...
c
        REAL*8          Nh, Msh
        common /mmatpar/ pr, Texp, Planck, Stefan, Nh,     Rh,     Rgas,
...
!     &                  ioform, iowflux, iofieldv, iotype, ioybar
     ...
     &                  ioform, iowflux, iofieldv, iotype, ioybar, 
     &                  ivort, icomputevort, 
     &                  nsynciofiles, nsynciofieldswriterestart, iMeshingTool
!MR CHANGE END
c
...
c----------------------------------------------------------------------
c
c.... common /errpar/   : element parameters
c
c numerr        : number of error statistics
c
c----------------------------------------------------------------------
c
...
c ivol          : output (element volumes)          [VOL.DAT]
c istat         : output (run status file)          [STATUS.DAT]
c
c----------------------------------------------------------------------
c
...
c----------------------------------------------------------------------
c
c.... common /matramp/   : data for ramping density and viscosity
c
c iramp 	: ramping flag
c tmu	  	: target viscosity
c trho          : target density
c qrts0         : global time - start of the ramp
c qrts1         :                   end
c nrts0		: global timestep - start of the ramp
c nrts1		: 		    end
c omu           : original viscosity
c orho          : original density
c
c----------------------------------------------------------------------
c
...
c iMeshingTool  : flag for either Chef or regular NSpre
c
c----------------------------------------------------------------------
c
