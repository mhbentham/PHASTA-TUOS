...
c  determine Stilde
        Stilde = vort + Sclr*fv2/(saKappa*dist2w)**2!unfix
c  determine r
        where(Stilde(:).ne.zero)
           r(:) = Sclr(:)/Stilde(:)/(saKappa*dist2w(:))**2
        elsewhere
           r(:) = 1.0d32
        endwhere
c  determine g
...
c        srcrat=saCb1*(one-ft2)*Stilde*Sclr
c     &      -(saCw1*fw - saCb1*ft2/saKappa**2)*(Sclr/dist2w)**2
c        srcrat=srcrat/Sclr

        srcrat=saCb1*(one-ft2)*Stilde
     &       -(saCw1*fw - saCb1*ft2/saKappa**2)*(Sclr/dist2w/dist2w)

c
c        term1=saCb1*(one-ft2)*Stilde*Sclr
c        term2=saCb2*saSigmaInv*(g1yti**2+g2yti**2+g3yti**2)
c        term3=-(saCw1*fw - saCb1*ft2/saKappa**2)*(Sclr/dist2w)**2
c determine d()/d(Sclr)
...
c  determine source term
     ...
     &      +saCb1*(one-ft2)*Stilde*Sclr
     &      -(saCw1*fw - saCb1*ft2/saKappa**2)*(Sclr/dist2w)**2
        bf = bf * rho
c determine d(source)/d(Sclr)
        srcp = rho*saCb1*(Sclr*stp+Stilde)
     &        -rho*saCw1*(fwp*Sclr**2 + 2*Sclr*fw)/dist2w**2
        do i=1, npro
