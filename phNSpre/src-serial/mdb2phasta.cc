...
// Read in the model file
    cout << "  pGModel model  " << endl;
    pGModel model = DM_load(fname, progress);
...
//    else 
     cout << "  AMAN_findCase(attmngr  " << endl;
        acase = AMAN_findCase(attmngr, "geom");
  if(acase) {
    cout<<"Found case, setting the model"<<endl;
    AttCase_setModel(acase,model);
    cout<<"Found case, setting the model .. done "<<endl;
  } else {
    cout<<"Case not found "<<endl;
    return 1;
  }
    cout<<"AttCase_associate(acase,progress);"<<endl;
  AttCase_associate(acase,progress);
    cout<<"AttCase_associate(acase,progress); ... done "<<endl;
...
//    AttCase_associate( acase ,progress);
    ...
    if (NSFTAG >= 0) {
        mesh = M_new(0,model);
#ifdef SIM
        mesh = M_load(mname,model,progress);
#else
        M_load(mesh,mname);
#endif
