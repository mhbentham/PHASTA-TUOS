...
!
     ...
     &     ithree,'integer'//char(0),iotype)
        ...
        if(ndofdwal .ne. 0) then 
          if(ndofdwal.ne.4) then
            warning='WARNING: ndofdwal not equal 4'
            write(*,*) warning, ndofdwal
          endif
          allocate( dwal(nshg,ndofdwal) )
          iqsiz=nshg*ndofdwal
...
!
      ...
      if(myrank.lt.irankN) then
        ...
        if(idwal == 1) then
          allocate(dwalN(nshgN,ndofdwal))   ! size of my final mapped field array
...
!                  write(*,*) '  ', irN,'receives',ifill,'data from',irM
     ...
     &                            qrecv( (i-1)*numvar+sumvar)
                    ...
                    if(idwal==1) then
                      do idof=1,ndofdwal
                        sumvar = 1+ndof+ndofybar+ndoferrors+
     &                           numphavg*ndofyphbar+ndofvort+idof
                        dwalN(intvID,idof) = qrecv( (i-1)*numvar+sumvar)
                      enddo
                    endif
                  enddo
                  deallocate(qrecv)
     ...
     &                                          vort(i,idof)
                      ...
                      if(idwal==1) then
                        do idof=1,ndofdwal
                           sumvar = 1+ndof+ndofybar+ndoferrors+
     &                              numphavg*ndofyphbar+ndofvort+idof
                           qsend( (ifill-1)*numvar+sumvar) =dwal(i,idof)
                        enddo
                      endif
                    endif
                  enddo
...
!
        ...
        do i=1,nshg
          ...
          intvID=(vID(i)+1.1)  !shift vID to start from 1
          if (isbinary == 1) then      
            write(700+ifileN) intvID, (dwal(i,j),j=1,ndofdwal)
          else
            write(700+ifileN,925) intvID, (dwal(i,j),j=1,ndofdwal)
          endif
!         the above "dealing" of records to files based on pID is safe because each 
!         process 1..M has a unique set of files to write to
        enddo
...
!
              ...
              do i=1, icountRead
                if(isbinary == 1) then
                  read(700+l) intvID,(qreadN(k),k=1,ndofRead) 
                else
                  read(700+l,925) intvID,(qreadN(k),k=1,ndofRead)
                endif
                dwalN(intvID,1:ndofdwal)=qreadN(1:ndofdwal) ! fill directly into dwalN
              enddo ! end of this map file so close the file and look for more
              deallocate(qreadN)
              close(700+l)
            endif ! map existed
