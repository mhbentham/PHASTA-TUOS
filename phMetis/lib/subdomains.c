 ...
 * George
 *
 * $Id: subdomains.c 658 2006-04-21 00:45:24Z benfrantzdale $
 *
 */
...
{
  int nvtxs, ndegrees;
  long long int me, i, j, k, nparts1, npartssq, edegr_pid, index; 
  idxtype *xadj, *adjncy, *adjwgt, *where;
  idxtype edegr_ed, pmat_i;
  RInfoType *rinfo;
  ...
  nparts1 = nparts;
  npartssq = nparts1*nparts1;

  idxset(npartssq, 0, pmat);
  ...
  for (i=0; i<nvtxs; i++) {
    if (rinfo[i].ed > 0) {
      ...
      for (j=0; j<ndegrees; j++) {
        edegr_pid = edegrees[j].pid;
        edegr_ed = edegrees[j].ed;
        index = k+edegr_pid;
        pmat_i = pmat[index];
        pmat[index] = pmat_i + edegr_ed;
      }
    }
  }
